...
c = ['k','b','c','r','m','g'];
    subplot(1,3,1)
    for k = 1:6
        plot(kf_vec,FE(:,k),c(k)); hold on;
        plot(kf_vec,FE(:,k) + FE_std(:,k),[c(k),'--']);
        plot(kf_vec,FE(:,k) - FE_std(:,k),[c(k),'--']); 
    end
    title('a)')
    ...
    subplot(1,3,2)
    for k = 1:6
        plot(kf_vec,FE1(:,k),c(k)); hold on;
        plot(kf_vec,FE1(:,k) + f1_std(:,k),[c(k),'--']);
        plot(kf_vec,FE1(:,k) - f1_std(:,k),[c(k),'--']); 
    end
    title('b)')
    xlabel('x','Interpreter','latex');
    ...
    subplot(1,3,3)
    for k = 1:6
        plotkP(k) = plot(kf_vec,KFE(:,k),c(k)); hold on;
        plot(kf_vec,KFE(:,k) + kf_std(:,k),[c(k),'--']);
        plot(kf_vec,KFE(:,k) - kf_std(:,k),[c(k),'--']); 
    end
    title('c)')
    xlabel('x','Interpreter','latex');
    ...
    save('salto freq\estimadoresFR_salto_freq_kf')