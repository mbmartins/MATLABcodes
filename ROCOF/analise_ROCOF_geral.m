...
for i = 0:(length(var_observada)-1)
    ...
    filename(8*i+2*j+2) = "geral\" + var_observada(i+1) + tipo_salto(j+1)+"_hist.png";
    end
end
...
f = 1; figB=figure(f); 
for k = 1:6
    subplot(subs(k)); plot(riraw_fase(k,:)); title(tits(k));
    xlabel('Samples'); ylabel('r_i [Hz/s]')
end
...
f = f+2; figB=figure(f);
for k = 1:6
    subplot(subs(k)); plot(riraw_mag(k,:)); title(tits(k));
    xlabel('Samples'); ylabel('r_i [Hz/s]')
end
...
f = f+2;figB=figure(f);
for k = 1:6
    subplot(subs(k)); plot(riraw_freq(k,:)); title(tits(k));
    xlabel('Samples'); ylabel('r_i [Hz/s]')
end
...
f = f+2;figB=figure(f); 
for k = 1:6
    subplot(subs(k)); plot(riraw_zero(k,:)); title(tits(k));
    xlabel('Samples'); ylabel('r_i [Hz/s]')
end
