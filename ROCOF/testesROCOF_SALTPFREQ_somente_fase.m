...
KfS = 0.05; %[Hz] %size of frequency step
Fs =4800;
NCycles = 6;
T = NCycles/F0;
...
mz = abs(z);
df = dfh.*mz./median(mz);
ROCOF_ih = gradient(dfh)/T;
ROCOF_i = gradient(df)/T;
...
ROCOF_PATV2 = gradient(fi_PATV);
figure; plot(fi_PATV); title('PATV-filtered f_i')
hold on; plot(df,'.'); %plot(ROCOF_PATV); 
legend('f_i-PATV','f_i'); %,'ROCOF_i')
figure; plot(ROCOF_PATV2); 
hold on; plot(ROCOF_i,'.'); title('estimated ROCOF_i')
%plot(v); 
legend('ROCOF_i-PATV','ROCOF_i')
