...
%    subplot(1,3,1)
    for k = 1:6
        pp(k) = plot(ka_vec,FE(:,k),c(k)); hold on;
        plot(ka_vec,FE(:,k) + FE_std(:,k),[c(k),'--']);
        plot(ka_vec,FE(:,k) - FE_std(:,k),[c(k),'--']); 
    end
%    title('a)')
    ...
    xlabel('$k_a$ [graus]')
    ylabel('$FE$ [Hz]')
    legend(pp, 'EF1','EF2','EF3','EF4','EF5','EF6')
    grid on
%     
%     subplot(1,3,2)
%     for k = 1:6
%         plot(ka_vec,FE1(:,k),c(k)); hold on;
%         plot(ka_vec,FE1(:,k) + f1_std(:,k),[c(k),'--']);
%         plot(ka_vec,FE1(:,k) - f1_std(:,k),[c(k),'--']); 
%     end
%     title('b)')
%     xlabel('x','Interpreter','latex');
...
%     subplot(1,3,3)
%     for k = 1:6
%         plotkP(k) = plot(ka_vec,KFE(:,k),c(k)); hold on;
%         plot(ka_vec,KFE(:,k) + kf_std(:,k),[c(k),'--']);
%         plot(ka_vec,KFE(:,k) - kf_std(:,k),[c(k),'--']); 
%     end
%     title('c)')
%     xlabel('x','Interpreter','latex');
