...
% Mag step
% Model: f(x) = x1*(1+x2u)*cos(w*t + phi)
...
for n = 1:Niter;
    %first guess
    %%%%       Amp  Kx  wf  ph
    par_var = [0.2 0.2 0.1 0.1]; % parameter variation in percent related to nominal
    xr = xnom1.*(1+(par_var/100).*(rand(1,length(xnom1))-0.5));
    
    %regenerates the signal with the uncertainties
        i = 1;
        Xm = xr(1); KxS(i) = xr(2); Wf(i) = xr(3); Ph(i) = xr(4);
        Ain = zeros(length(Xm),length(t));
        % Amplitude Step: applied after time passes 0
        Ain(i,:) = Xm(i);
        Ain(i,t >= 0) = Ain(i,t >= 0) * (1 + KxS(i));
...
MEAN_ERR = mean(errors)   %% ATENCAO: nao esta em [%]
