...
Pss = 90; %pior caso para salto mag phi = 0
...
%lambda = 0.11;
for k = 1:length(lambda_m); %para d=0
...
MCruns = 10000;
        ...
        for r = 1:MCruns  %taking the eps errors for tau1 only
            ...
            [tau_ep,dmax_p] = HD_PATV_estimator_optim(Signal,lambda_m(k),lambda_f,Lm,Lf); 
    ...
    message = "lambda:" + lambda_m(k)
    dmax_p_min(k) = min(dmax_p);
    dmax_p_mean(k) = mean(dmax_p);
end

save('otimiza_lambda_m.mat')    

%limiar_p_mean = mean(limiar_p)

% figure(2)
plot(lambda_m,eps_HD_PATV);
