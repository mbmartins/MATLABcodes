...
%          title('Point by point estimation error: Signal - y')
  ...
  %%%%% Intermediate phasors calculation
        T = NSamples*dt;
        tau_est2 = 0.25;
        tau_est5 = tau_pp + tau/NSamples;  %estimated tau/T (we can simulate errors in tau estimation)
        tau_est7 = 0.75;
        if KaS ~= 0
            %phase step
            Xe2_r = xr(1)/sqrt(2);   %reference magnitude (actual value xr)
...
[Synx_aw,Freq_aw,ROCOF_aw] = SteadyStateFit ( ...
	...
	Signal(NSamples/2+1:NSamples/2+500) ...
        ...
        Ve(6:10) = Synx_aw;
        Freq_LM(1:2) = Freq_bw;
        Freq_LM(3:5) = [X7(3)/(2*pi) X5(3)/(2*pi) X2(3)/(2*pi)]
        Freq_LM(6:10) = Freq_aw;
        Freq_SS(1:3) = Freq_bw;
