function [filename, std_R2, std_w, std_Xmag, std_Xphi] = MC_HLM4_par_hx(MCruns, par_var, SNR, Fs, hm, ha, phi0,run)
...
if run == true
    for ti = 1:9
        ...
        if ha ~= 0
            ...
            f = @(x) xnom(1)*cos(xnom(2)*t + xnom(3) + 2*pi + x(4)*(pi/180)*u);
        else
            %mag
            xnom = [Xm hm 2*pi*F1 Ph];
            f = @(x) xnom(1)*(1+x(2)*u).*cos(xnom(3)*t + xnom(4));
