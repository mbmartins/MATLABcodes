function [filename, std_R2, std_w, std_Xmag, std_Xphi] = MC_HLM4_par_Xm(MCruns, par_var, SNR, Fs, hm, ha, phi0,run)
% só há variação em Xm, os outros parms ficam constantes
...
if run == true
    for ti = 1:9
        ...
        if ha ~= 0
            ...
            f = @(x) x(1)*cos(xnom(2)*t + xnom(3) + 2*pi + xnom(4)*(pi/180)*u);
        else
            %mag
            %f = @(x) x(1)*(1+x(4)*u).*cos(x(2)*t + x(3));
            %xnom1 = [Vm 2*pi*F1 Ph KxS];
            xnom = [Xm hm 2*pi*F1 Ph];
            f = @(x) x(1)*(1+xnom(2)*u).*cos(xnom(3)*t + xnom(4));
