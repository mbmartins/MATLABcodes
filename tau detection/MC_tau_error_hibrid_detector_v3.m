function [tau_error,extremos,det_mag,det_fase,det_nan] = MC_tau_error_hibrid_detector_v3(SNR,Pin,lim_mag,lim_fase,tau_pp_set,Nruns)
...
for k = 1:Nruns
    ...
    [ifmax1, imax] = max(abs(df(br:end-br))); %max indices
    limiar1=lim_fase*median(abs(df(br:end-br)));
    
    gmi=gradient(abs(z)); % estimate of the instantaneous magnitude of z
    gmi=abs(gmi-median(gmi(br:end-br))); %v3mod
    [ifmax2, imax2] = max(abs(gmi(br:end-br))); %max indices
    ...
    if (ifmax1(1)>limiar1) && (ifmax2(1)>limiar2)
           ...
           if (ifmax1/limiar1) > (ifmax2/limiar2) %v3mod
                tau_e = tau_e1;
           else
               tau_e = tau_e2;
           end
