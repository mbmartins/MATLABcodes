...
kx_vec = [0.02:0.02:0.3];

for j = 1:length(kx_vec)
    %para acompanhamento
    j
    k_x = kx_vec(j);
    %----- Loop de Monte Carlo -----
...
c = ['k','b','c','r','m','g'];
    subplot(1,3,1)
    for k = 1:6
        plot(kx_vec,FE(:,k),c(k)); hold on;
        plot(kx_vec,FE(:,k) + FE_std(:,k),[c(k),'--']);
        plot(kx_vec,FE(:,k) - FE_std(:,k),[c(k),'--']); 
    end
    title('a)')
    ...
    subplot(1,3,2)
    for k = 1:6
        plot(kx_vec,FE1(:,k),c(k)); hold on;
        plot(kx_vec,FE1(:,k) + f1_std(:,k),[c(k),'--']);
        plot(kx_vec,FE1(:,k) - f1_std(:,k),[c(k),'--']); 
    end
    title('b)')
    xlabel('x','Interpreter','latex');
    ...
    subplot(1,3,3)
    for k = 1:6
        plotkP(k) = plot(kx_vec,KFE(:,k),c(k)); hold on;
        plot(kx_vec,KFE(:,k) + kf_std(:,k),[c(k),'--']);
        plot(kx_vec,KFE(:,k) - kf_std(:,k),[c(k),'--']); 
    end
    title('c)')
    xlabel('x','Interpreter','latex');
    ...
    save('salto mag\estimadoresFR_salto_mag_kf')