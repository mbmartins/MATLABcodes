...
for j = 1:length(Ncycles_vec)
    ...
    %----- Loop de Monte Carlo -----
    %MC_iterations = 300;
    [Fraw,f1raw,f2raw,kfraw] = MC_estimation(MCiter,F0,F1,Fs,phi_0,Ncycles_vec(j),tau_vec,tau2,SNR,k_a, k_x,k_f,nbits);
    F_mean = mean(Fraw,2);
...
filename = pasta+"estimadoresFR_salto_freq_T.mat";
    save(filename)
