function filename = teste_MedSF_Fs(MCiter,k_f,pasta);
...
for j = 1:length(Fs_vec)
    ...
    %----- Loop de Monte Carlo -----
    %MC_iterations = 300;
    [Fraw,f1raw,f2raw,kfraw] = MC_estimation(MCiter,F0,F1,Fs_vec(j),phi_0,NCycles,tau_vec,tau2,SNR,k_a, k_x,k_f,nbits);
    F_mean = mean(Fraw,2);
...
filename = pasta+"estimadoresFR_salto_freq_Fs.mat";
    save(filename)