function [f1,f2,f_r,f_u,ri,ru] = MedSF_PATV2(f_i, az, tau_n, lambda)
% MedSF_PATV com compensacao pre-truncamento
    ...
    %compensacao pre truncamento
    f_i = f_i.*az/median(az);
    f_i2 = f_i(brmask);
    %exclui as amostras proximo as bordas
    %f_utrunc = f_i(brmask);
    aztrunc = az(brmask);
    
    %compensacao apos o truncamento
    %f_i2 = f_utrunc.*aztrunc./median(aztrunc);
