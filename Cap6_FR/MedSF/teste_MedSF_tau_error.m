function filename = teste_MedSF_T(MCiter,k_f,pasta);
...
tau_n = floor(tau_nom*NSamples);
tau_vec = tau_n/NSamples;

for j = 1:length(tau_n_error)
    %para acompanhamento
    j

    %----- Loop de Monte Carlo -----
    %MC_iterations = 300;
    [Fraw,f1raw,f2raw,kfraw] = MC_estimation_tau_error(MCiter,F0,F1,Fs,phi_0,Ncycles,tau_nom,tau_n_error(j),tau2,SNR,k_a, k_x,k_f,nbits);
    F_mean = mean(Fraw,2);
...
filename = pasta+"estimadoresFR_salto_freq_tau_error.mat";
    save(filename)
