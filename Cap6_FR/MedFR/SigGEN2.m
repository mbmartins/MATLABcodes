function Signal = SigGEN2(F0,F1,SampleRate,Ps,NCycles,tau1,tau2,SNR, KaS, KxS,hf,nbits)
%creates a Signal with an integer number of samples for NCycles, with a
%step starting on tau1 and ending on tau2
% tau1 and tau2 are in proportion of total time
...
Wf1 = 2*pi*F1;  % fundamental frequency
...
%Freq step
Wf(i,1:NSamples) = Wf1;
Wf(i,t1 >= 0 & t2<=0) = Wf(i,t1 >= 0 & t2<=0) + hf*2*pi;
...
%Aruido = Vm/10^(SNR/20);
Signal = VSemulator(real(cSignal),nbits,SNR);